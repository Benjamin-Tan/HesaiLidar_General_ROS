...
// dual return
static std::vector<PPoint> PointCloudDual(MAX_POINT_CLOUD_NUM);
static int iPointCloudDualIndex = 0;

// single return ring filter
static std::vector<PPoint> PointCloudRingFilter(MAX_POINT_CLOUD_NUM);
static int iPointCloudRingFilterIndex = 0;
// dual return ring filter
static std::vector<PPoint> PointCloudDualRingFilter(MAX_POINT_CLOUD_NUM);
static int iPointCloudDualRingFilterIndex = 0;
          ...
          if ((last_azimuth_ > pkt.blocks[i].azimuth &&
            ...
            if (pcl_callback_ && (iPointCloudIndex > 0 || PointCloudList[0].size() > 0)) {
              ...
              if (iPointCloudDualIndex > 0) {
                EmitBackMessege(LASER_COUNT, outMsg, scan, 1);
              }
              if (iPointCloudRingFilterIndex > 0) {
                EmitBackMessege(LASER_COUNT, outMsg, scan, 2);
              }
              if (iPointCloudDualRingFilterIndex > 0) {
                EmitBackMessege(LASER_COUNT, outMsg, scan, 3);
              }
              scan->packets.clear();
            }
          }
...
void PandarGeneral_Internal::EmitBackMessege(char chLaserNumber, boost::shared_ptr<PPointCloud> cld, hesai_lidar::PandarScanPtr scan, int publisher_type) {
  if (pcl_type_) {
    for (int i=0; i<chLaserNumber; i++) {
    ...
    switch (publisher_type) {
      case 0:
        ...
        break;
      case 1:
        cld->points.assign(PointCloudDual.begin(),PointCloudDual.begin() + iPointCloudDualIndex);
        printf("1...%d\n", iPointCloudDualIndex);
        iPointCloudDualIndex = 0;
        break;
      case 2:
        cld->points.assign(PointCloudRingFilter.begin(),PointCloudRingFilter.begin() + iPointCloudRingFilterIndex);
        printf("2...%d\n", iPointCloudRingFilterIndex);
        iPointCloudRingFilterIndex = 0;
        break;
      case 3:
        cld->points.assign(PointCloudDualRingFilter.begin(),PointCloudDualRingFilter.begin() + iPointCloudDualRingFilterIndex);
        printf("3...%d\n", iPointCloudDualRingFilterIndex);
        iPointCloudDualRingFilterIndex = 0;
        break;
      default:
        break;
    }
